$(document).ready(function() {
  ...
  displayTime();
  console.log(buttons);  //created an Eventlistener
  buttons.on('click', function(event) {
    console.log("SAVE");
    event.preventDefault();
    const index = buttons.index(this);
    const apptInput = inputs.eq(index).val();
    console.log("Input: " + apptInput + " index: " + index);
    localStorage.setItem(`${index + 9} am`, JSON.stringify(apptInput));
    console.log(localStorage.getItem(`${index + 9} am`));
  });
  //created a funtion that renders the schedule
  const renderSchedule = index => {
    const scheduleDisplay = JSON.parse(localStorage.getItem(`${index + 9} am`)); '9 am'
    console.log("Schedule display: " + scheduleDisplay + " index: " + index);
